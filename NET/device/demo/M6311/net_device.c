		...
		switch(initStep)
		{
			...
			case 2:
				...
				if(dataPtr != NULL)
				{
					...
					if(dataPtr != NULL)
					{
						if(strstr((char *)netIOInfo.buf, "Basestation"))							//未发现基站
						{
							UsartPrintf(USART_DEBUG, "未发现基站，正在切换临近位置\r\n");
						...
						if(strstr((char *)netIOInfo.buf, "Balance"))								//余额不足
						{
							UsartPrintf(USART_DEBUG, "余额不足\r\n");
						...
						if(strstr((char *)netIOInfo.buf, "Unauthorized"))							//非法的apikey
						{
							UsartPrintf(USART_DEBUG, "非法的apikey\r\n");
